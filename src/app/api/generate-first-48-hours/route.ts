...
import { addStartupMemoryAdmin, getStartupMemoryAdmin } from "@/lib/server/startup-data";
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const { founderBackground, problemStatement, targetDemographic, industry, idea } = body;
...
Return ONLY valid JSON in this exact format:
{
    ...
    "criticalAssumptions": ["List of 3 key assumptions to validate"]
        ...
        for (const task of taskList) {
            ...
            createdTasks.push({ id: taskDoc.id });
        }

        await addStartupMemoryAdmin(
            startupId,
            "agent-output",
            "agent",
            JSON.stringify({
        ...
        return NextResponse.json(
            { error: { message: error.message || "Failed to generate First 48 Hours plan", code: "first48/failed" } },
            { status: 500 }
        );
    }
}
